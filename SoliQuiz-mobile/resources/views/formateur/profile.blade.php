...
@section('content')
<div x-data="formateurProfile()" x-init="init()">
    <!-- Header -->
    <header class="bg-white px-5 pt-safe-top pb-4 border-b border-slate-200 shadow-sm shrink-0 sticky top-0 z-40">
        ...
        <div class="flex items-center justify-between mt-2">
            <a href="{{ route('formateur.qcms') }}" class="text-slate-400 hover:text-slate-600">
                <svg class="size-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                    <path stroke-linecap="round" stroke-linejoin="round" d="M15 19l-7-7 7-7" />
                </svg>
            </a>
            <div class="flex-1 text-center">
                <span class="text-sm font-bold text-slate-800">Profil Formateur</span>
            </div>
            <div class="w-6"></div>
        </div>
    </header>
    ...
    <main class="flex-1 overflow-y-auto w-full hide-scrollbar pb-24 px-5 py-6">
        ...
        <div class="bg-white border border-slate-200 rounded-2xl shadow-sm overflow-hidden">
            <div class="px-5 py-4 border-b border-slate-100">
                <span class="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Email Professionnel</span>
