...
<nav class="fixed bottom-0 w-full max-w-[430px] bg-white border-t border-slate-200 z-50">
    <div class="flex justify-around items-center h-[72px] px-4 pb-safe">
        ...
        <a href="{{ route('student.bibliotheque') }}"
            class="flex flex-col items-center justify-center {{ request()->routeIs('student.bibliotheque') ? 'text-primary-500' : 'text-slate-400' }} gap-1.5 transition-colors">
            <svg class="size-6" fill="none" viewBox="0 0 24 24" stroke="currentColor"
                stroke-width="{{ request()->routeIs('student.bibliotheque') ? '2.5' : '2' }}">
                <path
            ...
            <span class="text-[10px] font-bold uppercase tracking-widest font-sans">Biblio</span>
        </a>
