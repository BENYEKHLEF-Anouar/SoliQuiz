...
@section('title', 'Classement - Apprenant')
        ...
        <div class="flex flex-col lg:flex-row lg:items-center justify-between gap-6 pb-8 border-b border-slate-200">
            <div>
                ...
                <p class="mt-2 text-sm text-slate-500 max-w-xl">
                    Visualisez les performances de votre classe et célébrez les réussites.
                </p>
            </div>
            ...
            <div class="flex flex-col sm:flex-row gap-4 items-end">
                <div class="w-full sm:w-72 relative">
                    <label class="block text-[10px] font-black uppercase tracking-widest text-slate-400 mb-2">Filtrer par QCM</label>
                    <div class="relative">
                        <button type="button" @click="openQcm = !openQcm" class="w-full bg-white border border-slate-200 rounded-xl px-4 py-3 text-xs font-bold text-slate-800 flex items-center justify-between outline-none focus:border-slate-900 transition-all shadow-xs text-left">
                            <span class="truncate pr-2">{{ $selectedQcm ? $selectedQcm->titre : 'Classement Général (Moyenne)' }}</span>
                        ...
                        <div x-show="openQcm" @click.away="openQcm = false" x-transition class="absolute z-[100] mt-2 w-full bg-white border border-slate-200 rounded-xl shadow-lg max-h-60 overflow-y-auto py-1" x-cloak>
                            ...
                            <button type="button" @click="openQcm = false; updateFilters({ qcm_id: '' })" class="w-full text-left px-4 py-2.5 text-xs font-bold text-slate-700 hover:bg-slate-50 hover:text-slate-900 flex items-center justify-between">
                            ...
                            @foreach($qcms as $q)
                                <button type="button" 
                                        ...
                                        @click="openQcm = false; updateFilters({ qcm_id: '{{ $q->id }}' })" class="w-full text-left px-4 py-2.5 text-xs font-bold text-slate-700 hover:bg-slate-50 hover:text-slate-900 flex items-center justify-between">
                                    <span class="truncate pr-2">{{ $q->titre }}</span>
                                    @if($qcmId == $q->id)
                                        <span>
                                            <svg class="size-4 text-primary-500" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="3"><path stroke-linecap="round" stroke-linejoin="round" d="m4.5 12.75 6 6 9-13.5"/></svg>
                                        </span>
                ...
                @if($qcmId)
                    <button type="button" @click="updateFilters({ qcm_id: '' })" class="size-11 bg-rose-50 text-rose-500 rounded-xl flex items-center justify-center hover:bg-rose-100 transition-all shadow-xs shrink-0" title="Réinitialiser les filtres">
            ...
            <table class="w-full text-left border-collapse">
                ...
                <tbody class="divide-y divide-slate-100">
                    @forelse($rankings as $row)
                        @php
                            $isMe = $row->id === Auth::id() || ($selectedQcm && $row->etudiant_id === Auth::id());
                        @endphp
                        <tr class="hover:bg-slate-50/60 transition-colors {{ $isMe ? 'bg-indigo-50/20' : '' }}">
                            <td class="px-6 py-4">
                                ...
                                <div class="flex items-center gap-3">
                                    ...
                                    <div>
                                        <span class="text-xs font-bold text-slate-800 flex items-center gap-1.5">
                                            ...
                                            @if($isMe)
                                                <span class="px-1.5 py-0.5 bg-indigo-100 text-indigo-700 rounded-sm text-[8px] font-black uppercase tracking-wider">Moi</span>
                                            @endif
                                        </span>
                                    </div>
                                </div>
                    ...
                    @empty
                        <tr>
                            <td colspan="10" class="px-6 py-12 text-center">
                                ...
                                <p class="text-xs text-slate-400 mt-1">Aucune tentative n'a été soumise pour le moment.</p>
                            </td>
                        </tr>
                    @endforelse
                </tbody>
