...
@section('content')
<div class="relative" x-data="pedagogieManager(
    '{{ $search ?? '' }}',
    '{{ $creatorFilter ?? '' }}',
    '{{ $creatorFilter && ($selectedCreator = $creators->find($creatorFilter)) ? addslashes($selectedCreator->prenom . ' ' . $selectedCreator->nom) : '' }}'
)">
    <!-- Ambient Background -->
    ...
    <!-- Filters & Search -->
    <div class="flex flex-col md:flex-row gap-4 mb-8 items-center justify-between relative z-20">
        <!-- Search Input -->
        <div class="flex-1 w-full relative group">
            ...
            <input x-model="search" @input.debounce.300ms="fetchContent()"
                ...
                type="text" placeholder="Rechercher par session, UA ou compétence...">
            
            <button type="button" x-show="search && !loading" @click="search = ''; fetchContent()" class="absolute right-5 top-1/2 -translate-y-1/2 text-slate-300 hover:text-slate-500 transition-colors" style="display: none;">
        ...
        <div class="relative w-full md:w-64" x-data="{ open: false, creatorSearch: '' }">
            <button @click="open = !open" type="button"
                    class="w-full h-[52px] flex items-center justify-between gap-2.5 px-4 bg-white border border-slate-200 rounded-xl text-xs font-black text-slate-600 uppercase tracking-wider hover:border-slate-400 hover:text-slate-900 transition-all shadow-sm"
                ...
                <span class="flex items-center gap-2">
                    ...
                    <span x-text="creatorId ? creatorName : 'Tous les créateurs'"></span>
                </span>
            ...
            <div x-show="open"
                 ...
                 class="absolute right-0 top-full mt-2 w-64 bg-white border border-slate-100 rounded-2xl shadow-2xl shadow-slate-200/60 z-50 overflow-hidden"
                 x-cloak>
                <div class="p-2 border-b border-slate-100">
                    <input type="text" x-model="creatorSearch" placeholder="Rechercher un formateur..." 
                           class="w-full bg-slate-50 border border-slate-200 rounded-xl px-3 py-2 text-xs font-bold text-slate-800 placeholder:text-slate-300 focus:bg-white focus:border-slate-400 focus:ring-0 outline-none transition-all">
                </div>
                <div class="p-2 max-h-64 overflow-y-auto">
                    <a href="#" @click.prevent="creatorId = ''; creatorName = ''; open = false; fetchContent();"
                       class="flex items-center gap-3 px-3 py-2.5 rounded-xl text-xs font-bold transition-all text-slate-600 hover:bg-slate-50"
                       :class="!creatorId ? 'bg-slate-900 text-white!' : ''">
                        <div class="size-7 rounded-lg bg-slate-100 flex items-center justify-center text-[9px] font-black" :class="!creatorId ? 'bg-white/20!' : ''">ALL</div>
                        Tous les créateurs
                    </a>
                    @foreach($creators as $creator)
                        <a href="#" @click.prevent="creatorId = '{{ $creator->id }}'; creatorName = '{{ addslashes($creator->prenom . ' ' . $creator->nom) }}'; open = false; fetchContent();"
                           x-show="!creatorSearch || '{{ strtolower(addslashes($creator->prenom . ' ' . $creator->nom)) }}'.includes(creatorSearch.toLowerCase())"
                           class="flex items-center gap-3 px-3 py-2.5 rounded-xl text-xs font-bold transition-all text-slate-600 hover:bg-slate-50"
                           :class="creatorId == '{{ $creator->id }}' ? 'bg-slate-900 text-white!' : ''">
                            <img class="size-7 rounded-lg border border-slate-100"
                                 :class="creatorId == '{{ $creator->id }}' ? 'border-white/30!' : ''"
                            ...
                            <div>
                                ...
                                <p class="text-[9px] font-black uppercase tracking-widest text-slate-400" :class="creatorId == '{{ $creator->id }}' ? 'opacity-60!' : ''">{{ ucfirst($creator->type_profil) }}</p>
                            </div>
                        </a>
                    @endforeach
                </div>
        ...
        <!-- Global Reset Button -->
        <button x-show="search || creatorId" @click="search = ''; creatorId = ''; creatorName = ''; fetchContent()"
                class="h-[52px] px-4 bg-rose-50 text-rose-500 rounded-xl font-bold text-xs hover:bg-rose-100 transition-colors flex items-center gap-2"
                style="display: none;">
    ...
    <!-- Quick Stats & Hierarchy Tree Container -->
    <div id="pedagogie-content" class="relative z-10" :class="loading ? 'min-h-[200px]' : ''">
        <!-- Loading Overlay -->
        <div x-show="loading"
