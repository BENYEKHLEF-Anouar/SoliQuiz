...
<div class="relative" x-data="{
    ...
    <div class="relative z-10 space-y-4">
        @forelse($seances as $seance)
            <div x-data="{ expanded: false, options: false }" class="group" :class="options ? 'z-[100]' : 'z-10'">
                <div class="bg-white/80 backdrop-blur-sm rounded-2xl border border-slate-100 shadow-sm hover:shadow-lg hover:border-slate-200 transition-all duration-300">
                            ...
                            <div class="flex items-center gap-6">
                                ...
                                <div class="flex items-center gap-2">
                                    <button @click.stop="openSeanceEdit({{ $seance->id }}, '{{ addslashes($seance->nom) }}', '{{ $seance->date_debut ? $seance->date_debut->format('Y-m-d') : '' }}', '{{ $seance->date_fin ? $seance->date_fin->format('Y-m-d') : '' }}', '{{ $seance->user_id }}')"
                                            type="button"
                                            class="size-10 rounded-xl bg-slate-50 text-slate-400 hover:bg-primary-500 hover:text-white hover:shadow-lg hover:-translate-y-0.5 hover:shadow-primary-500/20 transition-all flex items-center justify-center group/edit">
                                        <svg class="size-4 group-hover/edit:scale-110 transition-transform" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="3"><path d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z" /></svg>
                                    </button>

                                    <div class="relative">
                                        <button @click.stop="options = !options" @click.away="options = false" type="button"
                                                class="size-10 rounded-xl bg-slate-50 text-slate-400 hover:bg-slate-200 hover:text-slate-900 transition-all flex items-center justify-center">
                                        ...
                                        <div x-show="options" 
                                             class="absolute bottom-full right-0 mb-3 w-48 bg-white rounded-2xl border border-slate-100 shadow-premium z-50 py-2 overflow-hidden"
                        ...
                        <div class="p-5 md:p-6">
                            ...
                            <div class="space-y-2 pl-3 border-l-2 border-slate-200">
                                @forelse($seance->unitesApprentissage as $ua)
                                    <div x-data="{ expandedUa: false, optionsUa: false }" class="group/ua" :class="optionsUa ? 'z-[50]' : 'z-10'">
                                            ...
                                            <div class="flex items-center justify-between">
                                                ...
                                                <div class="flex items-center gap-2">
                                                    <button @click.stop="openUaEdit({{ $ua->id }}, '{{ addslashes($ua->nom) }}', '{{ addslashes($ua->code) }}', '{{ $ua->date_debut ? $ua->date_debut->format('Y-m-d') : '' }}', '{{ $ua->date_fin ? $ua->date_fin->format('Y-m-d') : '' }}', '{{ $ua->user_id }}')"
                                                            type="button"
                                                            class="size-8 rounded-lg bg-slate-50 text-slate-400 hover:bg-primary-500 hover:text-white transition-all flex items-center justify-center group/edit">
                                                        <svg class="size-3.5 group-hover/edit:scale-110 transition-transform" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.5"><path d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z" /></svg>
                                                    </button>
                                                    
                                                    <div class="relative">
                                                        <button @click.stop="optionsUa = !optionsUa" @click.away="optionsUa = false" type="button"
                                                                class="size-8 rounded-lg bg-slate-50 text-slate-400 hover:bg-slate-200 hover:text-slate-900 transition-all flex items-center justify-center">
                                                        ...
                                                        <div x-show="optionsUa" 
                                                             class="absolute bottom-full right-0 mb-2 w-40 bg-white rounded-xl border border-slate-100 shadow-premium z-50 py-1.5 overflow-hidden"
                                                             x-transition:enter="transition ease-out duration-200" 
                                            ...
                                            <div x-show="expandedUa" x-collapse class="mt-3 pt-3 border-t border-slate-50">
                                                ...
                                                <div class="grid grid-cols-1 sm:grid-cols-2 gap-2">
                                                    @forelse($ua->competences as $comp)
                                                        <div x-data="{ optionsComp: false }" :class="optionsComp ? 'z-[50]' : 'z-10'" class="bg-emerald-50 p-2.5 rounded-lg border border-emerald-100 flex justify-between items-center group/comp relative">
                                                            <div class="flex items-center gap-2">
                                                            ...
                                                            <div class="flex items-center gap-1.5 opacity-0 group-hover/comp:opacity-100 transition-opacity">
                                                                <button @click.stop="openCompEdit({{ $comp->id }}, '{{ addslashes($comp->libelle) }}', '{{ addslashes($comp->code) }}')"
                                                                        type="button"
                                                                        class="p-1.5 text-emerald-500 hover:bg-emerald-100 rounded-md transition-colors">
                                                                    <svg class="size-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.5"><path d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z" /></svg>
                                                                </button>
                                                                
                                                                <div class="relative">
                                                                    <button @click.stop="optionsComp = !optionsComp" @click.away="optionsComp = false" type="button"
                                                                            class="p-1.5 text-slate-400 hover:bg-slate-100 rounded-md transition-colors">
                                                                        <svg class="size-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="3"><path d="M12 5v.01M12 12v.01M12 19v.01M12 6a1 1 0 110-2 1 1 0 010 2zm0 7a1 1 0 110-2 1 1 0 010 2zm0 7a1 1 0 110-2 1 1 0 010 2z" /></svg>
                                                                    </button>

                                                                    <div x-show="optionsComp" 
                                                                         class="absolute bottom-full right-0 mb-1 w-32 bg-white rounded-xl border border-slate-100 shadow-premium z-50 py-1 overflow-hidden"
                                                                         x-transition:enter="transition ease-out duration-150" 
                                                                        ...
                                                                        <button type="button" 
                                                                                ...
                                                                                class="w-full flex items-center gap-2 px-3 py-1.5 text-rose-500 hover:bg-rose-50 transition-colors text-[8px] font-black uppercase tracking-widest">
                                                                            <svg class="size-3" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.5"><path d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" /></svg>
                                                                            <span>Supprimer</span>
                                                                        </button>
        ...
        <div>
            <x-ui.modal name="seance-modal" maxWidth="md">
                ...
                <form x-bind:action="seanceMode === 'create' ? '{{ route('admin.pedagogie.seance.store') }}' : `{{ url('/admin/pedagogie/seance') }}/${seanceId}`" method="POST" class="space-y-4">
                    ...
                    <div x-data="{ open: false, label: 'Sélectionner un formateur' }" 
                        ...
                        <button type="button" @click="open = !open" @click.away="open = false"
                            ...
                            <svg class="size-4 text-slate-400 transition-transform" :class="open ? 'rotate-180' : ''" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="3">
                                <path d="M19 9l-7 7-7-7"/>
                            </svg>
                        </button>
                        ...
                        <div x-show="open" 
                            ...
                            @foreach($formateurs as $formateur)
                                <button type="button" 
                                        ...
                                        @click="seanceFormateurId = '{{ $formateur->id }}'; label = '{{ $formateur->prenom }} {{ $formateur->nom }}'; open = false"
                    ...
                    <div x-data="{ open: false, label: 'Sélectionner un formateur' }" 
                        ...
                        <div x-show="open" 
                            ...
                            @foreach($formateurs as $formateur)
                                <button type="button" 
                                        data-id="{{ $formateur->id }}"
                                        @click="uaFormateurId = '{{ $formateur->id }}'; label = '{{ $formateur->prenom }} {{ $formateur->nom }}'; open = false"
                                        class="w-full px-4 py-2.5 text-left text-sm font-bold text-slate-600 hover:bg-slate-50 hover:text-primary-600 transition-colors flex items-center gap-3">
                                    <div class="size-6 rounded bg-slate-100 flex items-center justify-center text-[9px] font-black text-slate-400">
                                        {{ substr($formateur->prenom, 0, 1) }}{{ substr($formateur->nom, 0, 1) }}
                                    </div>
