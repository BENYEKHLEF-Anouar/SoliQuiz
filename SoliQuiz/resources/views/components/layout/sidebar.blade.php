        ...
        <nav class="flex-1 px-3 py-6 space-y-6 overflow-y-auto custom-scrollbar">
            @php
                $categories = [
                    ...
                    'etudiant' => [
                        'Espace Apprenant' => [
                            ['name' => 'Tableau de Bord', 'route' => 'student.dashboard', 'icon' => 'M3 13h8V3H3v10zm0 8h8v-6H3v6zm10 0h8V11h-8v10zm0-18v6h8V3h-8z'],
                            ['name' => 'Bibliothèque QCM', 'route' => 'student.bibliotheque', 'icon' => 'M12 6.253v13m0-13C10.832 5.477 9.246 5 7.5 5S4.168 5.477 3 6.253v13C4.168 18.477 5.754 18 7.5 18s3.332.477 4.5 1.253m0-13C13.168 5.477 14.754 5 16.5 5c1.747 0 3.332.477 4.5 1.253v13C19.832 18.477 18.247 18 16.5 18c-1.746 0-3.332.477-4.5 1.253'],
                        ]
                    ]
                ];
            ...
            @foreach($currentRoleCategories as $category => $categoryLinks)
                <div>
                    <p class="text-[10px] font-black text-slate-400 uppercase tracking-[0.3em] mb-4 flex items-center gap-2 italic px-4 leading-none">
                        {{ $category }}
                    </p>
                    <div class="space-y-1">
                        @foreach($categoryLinks as $link)
                            ...
                            <a href="{{ route($link['route']) }}"
                                class="group flex items-center gap-3.5 px-5 py-3 rounded-xl transition-all duration-200 {{ $isActive ? 'bg-primary-500 text-white shadow-lg shadow-primary-500/20 italic' : 'text-slate-500 hover:bg-primary-50 hover:text-primary-600 italic' }}">
